...
class MapCubit extends Cubit<MapState> {
  ...
  void setMapViewScheme(MapScheme mapScheme) {
    state.mapController?.mapScene.loadSceneForMapScheme(mapScheme, (MapError? error) {
      if (error != null) {
  ...
  searchLocation(String text, GeoCoordinates startLocCoordinate) {
    _truckExampleInstance?.searchLocation(text).then((value) {
      emit(state.copyWith(suggestionDestination: value));
    });
  }

  void setDestinationCoordinate(GeoCoordinates coordinate) {
    _truckExampleInstance?.destinationGeoCoordinates = coordinate;
    emit(state.copyWith(destinationCoordinates: coordinate));
  }

  void setRoute() {
    if (state.startCoordinates == null || state.destinationCoordinates == null) return;
    _truckExampleInstance?.onShowRouteButtonClicked(state.startCoordinates!, state.destinationCoordinates!, onGetAvailableRoutes);
    emit(state.copyWith(mapController: state.mapController));
  }

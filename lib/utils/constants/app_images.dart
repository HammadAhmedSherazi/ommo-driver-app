part of 'constants.dart';

class AppImages {
  AppImages._();

  static const String imagePath = 'assets/images/';

  static const String defaultMapImg = "${imagePath}default_map_img.png";
  static const String redMapPin = "${imagePath}red_map_pin.png";
  static const String redLocationIcon = "${imagePath}destination_pin (2).png";
  static const String greenMarker = "${imagePath}green_marker.png";
  static const String greenMapPin = "${imagePath}green_map_pin.png";
  static const String satelliteMapImg = "${imagePath}satellite_map_img.png";
  static const String logoIcon = "${imagePath}logo_icon.png";
  static const String filterIcon = "${imagePath}mdi_filter.png";
  static const String bookmarkIcon = "${imagePath}bookmark.png";
  static const String blackWhitLocationIcon =
      "${imagePath}black_white_location.png";
  static const String arrowForward = "${imagePath}arror_forwart.png";
  static const String suggestionPin = "${imagePath}suggestion_pin.png";
  static const String startingPointIcon = "${imagePath}Icon (28).png";
  static const String destinationPointIcon = "${imagePath}Icon (29).png";
  static const String destinationPointPin = "${imagePath}destination_pin.png";
  static const String stopPointIcon = "${imagePath}stop_point_icon.png";
  static const String pointTrailingIcon = "${imagePath}stop_point_icon.png";
  static const String pointsShuffleIcon = "${imagePath}arrow-down-up.png";
  static const String pointDragIcon = "${imagePath}si_drag-handle-duotone.png";
  static const String addCircle = "${imagePath}Frame.png";
  static const String blackPointPin = "${imagePath}black_map_pin.png";
  
  
  
  static const String loveLogo = "${imagePath}image 2.png";
  static const String kwikTripLogo = "${imagePath}kwik_trip_icon.png";
  static const String pilotLogo = "${imagePath}Circle.png";
  static const String taPetroLogo = "${imagePath}ta_petro_icon.png";
  

  
}
